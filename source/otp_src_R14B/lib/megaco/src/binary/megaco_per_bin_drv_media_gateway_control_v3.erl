...
?RT_PER:encode_open_type(dummy,?RT_PER:complete(  %%INTEGER with effective constraint: [{'ValueRange',{0,99},100,{bits,7}}]
  begin
  case Tmpval7 of
    Tmpval8 when Tmpval8=<99,Tmpval8>=0 ->
      [10,7,Tmpval8- 0];
    Tmpval8 ->
      exit({error,{value_out_of_bounds,Tmpval8}})
  end
...
?RT_PER:encode_bit_string(no,Tmpval7,[{onTimeOut,0},{onInterruptByEvent,1},{onInterruptByNewSignalDescr,2},{otherReason,3},{onIteration,4}])
end,
case element(7,Val1) of
asn1_NOVALUE -> [];
Tmpval8 ->
...
?RT_PER:encode_open_type(dummy,?RT_PER:complete(  %%INTEGER with effective constraint: [{'ValueRange',{0,65535},65536,{octets,2}}]
  begin
  case Tmpval11 of
    Tmpval12 when Tmpval12=<65535,Tmpval12>=0 ->
      [20,2,<<(Tmpval12- 0):16>>];
    Tmpval12 ->
      exit({error,{value_out_of_bounds,Tmpval12}})
  end
...
?RT_PER:encode_open_type(dummy,?RT_PER:complete(case Tmpval3 of
'test' -> [0,10,2,0];
'outOfSvc' -> [0,10,2,1];
'inSvc' -> [0,10,2,2];
...
?RT_PER:encode_open_type(dummy,?RT_PER:complete(  %%INTEGER with effective constraint: [{'ValueRange',{0,15},16,{bits,4}}]
  begin
  case Tmpval6 of
    Tmpval7 when Tmpval7=<15,Tmpval7>=0 ->
      [10,4,Tmpval7- 0];
    Tmpval7 ->
...
?RT_PER:encode_open_type(dummy,?RT_PER:complete(case Tmpval8 of
  true -> [1];
  false -> [0];
  _ -> exit({error,{asn1,{encode_boolean,Tmpval8}}})
end))
